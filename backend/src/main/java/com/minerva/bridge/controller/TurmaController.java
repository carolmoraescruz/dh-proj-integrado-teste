...
import com.minerva.bridge.service.TurmaService;
...
public class TurmaController {
	...
	public ResponseEntity<List<Turma>> findAll() {
		List<Turma> listaTurmas = turmaService.findAll();
	...
	public ResponseEntity<Turma> insert(@RequestBody Turma obj) {
		...
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(obj.getIdTurma()).toUri();
	...
	public ResponseEntity<Turma> update(@PathVariable Long id, @RequestBody Turma obj) {
		obj = turmaService.update(id, obj);
		return ResponseEntity.ok().body(obj);
	}
